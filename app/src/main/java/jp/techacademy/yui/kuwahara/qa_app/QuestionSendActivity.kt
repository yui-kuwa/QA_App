...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    companion object {
        //それぞれ100として定数を定義してるけど何でもいい
        //このActivity内で複数のパーミッションの許可のダイアログを出すことがある場合、
    ...
    override fun onClick(v: View) {
        ...
        if (v === imageView) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {//6.0以降の場合
                //外部ストレージへの書き込みが許可されているか
                if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    // 許可されている
                    showChooser()//Intent連携でギャラリーとカメラを選択するダイアログを表示させる
                } else {
            ...
            //保存　　第2引数にはCompletionListenerクラスを指定->保存するのに時間を要することが予想されるのでCompletionListenerクラスで完了を受け取る
            genreRef.push().setValue(data, this)
            progressBar.visibility = View.VISIBLE
        }
    }

    //許可ダイアログでユーザが選択した結果を受け取る関数
    ...
    private fun showChooser() {
        ...
        //第1引数に1つ目のIntentを指定し、第2引数にはダイアログへ表示するタイトルを指定
    ...
    override fun onComplete(databaseError: DatabaseError?, databaseReference: DatabaseReference) {
        ...
        if (databaseError == null) {//Firebaseへの保存が完了
            finish()//Activityを閉じる
        } else {//Firebaseへの保存失敗
            //Snackbarでエラーの旨を表示
